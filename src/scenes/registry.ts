...
export const SCENES: SceneDef[] = [
  {
    ...
    notes: [
      ...
      'Hard cuts only at 21 s (footage → photo), 73 s (memory → training) and 81 s (training → loop).',
      'No layer stamps: fast = continuous orange screening, slow = selective blue reasoning.',
      'Numbers on screen: SAM 3.1 bead score, log clock, 119–182 A window (once, chapter 5). Everything else is words on the data.',
